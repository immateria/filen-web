       ...
       const loadHexChunk = useCallback(
               async ({ itm, start = 0 }: { itm: DriveCloudItem; start?: number }) => {
                       ...
                       try {
                               ...
                               if (start === currentOffset + existing.length) {
                                       buffer = Buffer.concat([existing, chunk])
                                       if (buffer.length > MAX_HEX_BUFFER_SIZE) {
                                               buffer = buffer.subarray(HEX_CHUNK_SIZE)
                                               offset += HEX_CHUNK_SIZE
                                       }
                               } else if (start + chunk.length === currentOffset) {
                                       buffer = Buffer.concat([chunk, existing])
                                       if (buffer.length > MAX_HEX_BUFFER_SIZE) {
